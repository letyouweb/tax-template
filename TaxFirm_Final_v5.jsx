...
const PremiumTaxFirm = () => {
  ...
  return (
    <div className="font-sans text-slate-800 bg-white selection:bg-amber-100 selection:text-amber-900">
      ...
      {/* 네비게이션 */}
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? 'bg-slate-900/95 backdrop-blur-md py-4 shadow-xl' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="text-white font-serif text-2xl tracking-widest font-bold z-50">
            YOO DONG SU
            <span className="block text-xs font-sans font-light tracking-[0.3em] text-amber-400 mt-1">TAX & ACCOUNTING</span>
          </div>
          
          <div className="hidden md:flex items-center space-x-12">
            {['회사소개', '인사이트', '전문분야', '상담문의'].map((item) => (
              <a key={item} href="#" className="text-sm font-light text-slate-200 hover:text-amber-400 transition-colors tracking-wide">
                {item}
              </a>
            ))}
            <button className="px-6 py-2 border border-amber-500 text-amber-500 rounded-sm hover:bg-amber-500 hover:text-white transition-all text-sm tracking-wide">
              전화 상담
            </button>
          </div>
      ...
      <main className="snap-container bg-slate-50">
        ...
        {/* 1. Hero Section - 2단 레이아웃 (텍스트 좌측 / 사진 우측) */}
        <section className="snap-section relative flex items-center bg-slate-900 text-white overflow-hidden">
          <div className="container mx-auto px-6 py-24 grid md:grid-cols-2 gap-12 items-center min-h-screen">
            {/* 왼쪽 컬럼: 텍스트 영역 */}
            <div className="space-y-8">
              <p className="text-sm tracking-[0.3em] text-amber-400 font-semibold">
                YOO DONG SU · TAX & ACCOUNTING
              </p>
              <h1 className="font-serif text-4xl md:text-5xl leading-snug">
                국세청 <span className="text-amber-400">25년</span>,<br />
                세무조사·양도·상속·증여 전문
              </h1>
              <p className="text-lg md:text-xl text-slate-200 leading-relaxed">
                복잡한 세무는 <span className="font-semibold text-white">유동수 세무회계</span>에 맡기시고,<br />
                <span className="bg-amber-500/15 px-1.5 py-0.5 rounded text-amber-200">
                  대표님은 사업에만 집중하세요.
                </span>
              </p>

              <div className="flex flex-col md:flex-row gap-4 pt-4">
                <button className="px-10 py-4 bg-amber-600 text-white rounded-sm hover:bg-amber-700 transition-all flex items-center justify-center gap-3 shadow-lg shadow-amber-900/30 group">
                  <span className="font-medium tracking-wide text-lg">프리미엄 1:1 상담 예약</span>
                  <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </button>
                <button className="px-10 py-4 border border-slate-400 text-slate-300 rounded-sm hover:border-white hover:text-white transition-all flex items-center justify-center gap-3 hover:bg-white/5">
                  <Shield size={20} />
            ...
            {/* 오른쪽 컬럼: 대표 사진 (흑백 + 좌측 페이드) */}
            <div className="relative">
              <div className="absolute inset-y-0 left-0 w-1/3 bg-gradient-to-l from-transparent to-slate-900 z-10" />
          ...
          {/* 하단 성과 바 (Stats Bar) */}
          <div className="absolute bottom-0 w-full border-t border-white/10 bg-slate-950/90 backdrop-blur-md z-20">
            <div className="container mx-auto px-6 py-6">
              <div className="grid grid-cols-3 gap-4 md:gap-8 text-center divide-x divide-white/10">
                <div className="space-y-1">
                  <div className="text-3xl md:text-4xl font-serif text-amber-400 font-bold">25<span className="text-xl">+</span></div>
                  <div className="text-xs md:text-sm text-slate-400 tracking-wider">국세청·세무 경력 (년)</div>
                </div>
                <div className="space-y-1">
                  <div className="text-3xl md:text-4xl font-serif text-amber-400 font-bold">3,000<span className="text-xl">+</span></div>
                  <div className="text-xs md:text-sm text-slate-400 tracking-wider">누적 상담 건수</div>
                </div>
                <div className="space-y-1">
                  <div className="text-3xl md:text-4xl font-serif text-amber-400 font-bold">100<span className="text-xl">억+</span></div>
                  <div className="text-xs md:text-sm text-slate-400 tracking-wider">절세·감면 달성액</div>
                </div>
              </div>
            </div>
          </div>
              ...
              {[
                {
                  ...
                  title: "세무조사",
                  sub: "통지서를 받았다면",
                  desc: "당황해서 전화부터 하지 마세요. 자료 준비와 정리부터 차근차근 안내해 드립니다."
                },
                {
                  icon: <TrendingUp size={32} />,
                  title: "양도소득세",
                  sub: "아파트·상가 양도 전",
                  desc: "판매 가격만 보지 마세요. 보유 기간, 거주 여부에 따른 진짜 수익을 계산해 드립니다."
                },
                {
                  icon: <Users size={32} />,
                  title: "상속·증여",
                  sub: "부모 재산 상속 전",
                  desc: "상속은 '돌아가신 뒤'가 늦습니다. 가족 분쟁을 줄이는 사전 설계를 함께합니다."
                },
                {
                  icon: <Lock size={32} />,
                  title: "조세불복",
                  sub: "억울한 세금 고지",
                  desc: "모든 세금이 정당하진 않습니다. 조사과 경험으로 다툴 수 있는지 먼저 검토합니다."
                }
              ].map((card, idx) => (
                <div key={idx} className="bg-white p-8 rounded-sm shadow-md hover:shadow-xl hover:-translate-y-2 transition-all duration-300 border-t-4 border-transparent hover:border-amber-500 group">
                  ...
                  <p className="text-sm text-slate-500 leading-relaxed word-keep break-keep">
                    {card.desc}
                  </p>
                </div>
              ))}
        ...
        {/* 3. Representative Services (핵심 솔루션 - RSS 제거 후 바로 연결) */}
        <section className="snap-section flex items-center bg-slate-900 text-white">
          <div className="container mx-auto px-6 py-20">
            ...
            <div className="grid md:grid-cols-2 gap-8 md:gap-12 max-w-5xl mx-auto">
              {/* Service 1 */}
              <div className="bg-slate-800 p-10 rounded-sm border border-slate-700 hover:border-amber-500 transition-colors group relative overflow-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
              <div className="relative">
                ...
                <img 
                  ...
                  alt="유동수 세무사" 
                  className="relative z-10 w-full shadow-2xl grayscale object-cover"
                />
                <div className="absolute bottom-10 -right-10 bg-white p-6 shadow-xl z-20 hidden md:block max-w-xs border-l-4 border-amber-500">
                  <p className="font-serif italic text-slate-700 text-sm leading-relaxed">
                    "세금은 피할 수 없지만, 억울한 세금까지 감당할 필요는 없습니다. 정확한 법과 사실을 바탕으로 고객이 납득할 수 있는 결과를 만듭니다."
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-8 pl-0 md:pl-10">
                ...
                <div className="space-y-4 text-slate-600 leading-relaxed text-sm md:text-base border-t border-slate-200 pt-6">
                  <p>
                    국세청에서 25년 동안 세무 현장을 지켜봤습니다.<br/>
                    강남세무서 조사과 사무관으로 근무하며 수많은 세무조사와 조세불복 사건을 처리했습니다.
                  </p>
                  <p>
                    이제는 그 경험을 바탕으로, 납세자 편에서 세무조사·양도·상속·증여 문제를 함께 풀어가고자 합니다.
                  </p>
                </div>
        ...
        {/* 5. Location / Contact Section – 오시는 길 */}
        <section className="snap-section flex items-center bg-slate-50">
          <div className="container mx-auto px-6 py-20 grid md:grid-cols-2 gap-12 items-center max-w-6xl">
            {/* 텍스트 영역 */}
            <div className="space-y-6">
              <div>
                <h4 className="text-amber-600 font-bold text-sm tracking-widest mb-2">
                  LOCATION
                </h4>
                <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">
                  오시는 길
              ...
              <div className="space-y-4 text-sm md:text-base text-slate-700 leading-relaxed">
                <div>
                  <p className="font-semibold">강남구청역 2번 출구</p>
                  <p>도보 6분거리에 있습니다.</p>
                </div>
                <div>
                  <p className="font-semibold">상담전화</p>
                  <p>02-518-0130</p>
                </div>
                <div>
                  <p className="font-semibold">영업시간</p>
                  <p>평일 오전 9:30 ~ 오후 5:30</p>
                  <p>주말 정기 휴무</p>
                </div>
                <div>
                  <p className="font-semibold">주소안내</p>
            ...
            <div className="relative">
              ...
              <img
                ...
                alt="유동수 세무회계 오시는 길"
                className="relative z-10 w-full h-[260px] md:h-[360px] object-cover rounded-lg shadow-2xl"
              />
            </div>
          </div>
        </section>
        ...
        {/* 6. CTA (문의) */}
        <section className="snap-section flex items-center justify-center bg-slate-900 text-center relative overflow-hidden">
           {/* 은은한 배경 패턴 */}
           <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5"></div>
           <div className="container relative z-10 px-6 space-y-10">
              <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                복잡한 세금 문제,<br/>
                <span className="text-amber-400">골든타임</span>을 놓치지 마세요.
              </h2>
              <div className="flex flex-col md:flex-row justify-center gap-6 mt-8">
                <button className="px-12 py-5 bg-amber-600 text-white font-bold rounded-sm hover:bg-amber-500 transition-all text-lg shadow-xl shadow-amber-900/40">
                  1:1 상담 예약하기
                </button>
           ...
           <footer className="absolute bottom-6 w-full text-center text-slate-500 text-xs">
